...
{
	FString SerializedLevel = "";
	int ColumnSize = GridResolution;
	int CurrIndex = 0;
	SerializedLevel.AppendInt(ColumnSize);
	SerializedLevel.AppendChar('\n');
	for (ABloxGridTile* GridTile : FloorArray)
	{
		...
		else
		{
			...
			SerializedLevel.AppendChar(TypeChar);
		}
		++CurrIndex;
	}

	UE_LOG(LogTemp, Warning, TEXT("Serialized Level Output: "));
	UE_LOG(LogTemp, Warning,TEXT("%s"), *SerializedLevel);
}
